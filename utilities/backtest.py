...
def portfolio_returns(
    ...
    if transaction_costs != 0.0:
        # Compute the absolute weight changes at each rebalance date
        weight_changes = aligned_portfolios.diff().fillna(0.0)

        # Turnover one way is the sum of absolute weight changes divided by 2 (since buying and selling are both counted)
        one_way_turnover = weight_changes.abs().sum(axis=1) / 2.0

        gross_returns = gross_returns - (one_way_turnover * transaction_costs)

    return gross_returns  #!!! COMPLETE AS APPROPRIATE !!!
